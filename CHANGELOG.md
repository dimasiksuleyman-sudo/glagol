...
- **Индикатор диктовки дожидается готовности микрофона.** Раньше пилюля
  ...
  начатая по этому сигналу, могла терять первое слово. Теперь сначала показана
  «Подготовка микрофона…», а красная точка и полоски появляются после первого
  аудиопакета. Первые данные сохраняются целиком, отпускание клавиши во время
  подготовки отменяет попытку без распознавания, а отсутствие аудиоданных
  после открытия потока завершается ошибкой через 5 секунд.

## [v0.2.1] — 2026-07-19

### Fixed
- **Приложение больше не запускается в нескольких копиях.** Раньше повторный
  запуск ярлыка открывал ещё одну копию Glagol — каждая жила в трее и в
  диспетчере задач, и каждая при старте пыталась занять глобальную горячую
  клавишу диктовки (`Ctrl+Shift+Space`), из-за чего вторая копия либо не
  получала хоткей, либо перехватывала его у первой. Теперь работает только одна
  копия: повторный запуск возвращает уже открытое окно (в том числе из трея и из
  свёрнутого состояния) вместо создания нового процесса.

## [v0.2.0] — 2026-07-18

### Added
- **Диктовка (голосовой ввод)** — новый раздел «Диктовка» в меню. Удерживайте
  горячую клавишу, говорите, отпустите — распознанный текст вставляется в
  активное окно. На странице настраиваются:
  - **Режим вставки** — «Автовставка» (Ctrl+V автоматически) или «Только буфер
    обмена».
  - **Горячая клавиша** — по умолчанию `Ctrl+Shift+Space`; кнопка «Изменить»
    ловит нажатую комбинацию (с ручным вводом на случай, если комбинация не
    распозналась). Занятая другим приложением комбинация откатывается на
    прежнюю.
  - **Выбор микрофона** — системный по умолчанию или конкретное устройство;
    недоступное сохранённое устройство помечается.
  - **История диктовки** — по умолчанию выключена; хранит до 10 последних
    расшифровок с превью, раскрытием и кнопкой «Копировать». Кнопка «Очистить
    историю» удаляет накопленное.
  - **Надиктовано всего** — счётчик минут за всё время.
  - Распознавание через OpenAI-совместимый провайдер (ключ и адрес — в
    Настройки → «Диктовка (STT)»), работает без системного VPN.
- **Раздел «Диктовка» в руководстве пользователя** — краткое описание и единый
  список из семи известных ограничений (буфер, окна администратора, шумная
  среда, конфликт хоткея в Office и др.), билингва RU + EN.
